...
The brief is written to give the review something to do. The vendor, posture, and hosting region confirm cleanly. The renewal date is genuinely contested — the brief's own open-questions section records a different date on the signature page — so the reviewer has to judge which one the schedule supports. The security assurance names a report that has not been delivered, which is what `Could not confirm` is for, and the export stays refused until that item is resolved. `examples/vendor-obligations/source-revised.txt` is the amended brief: pass it to `fieldwork recheck` and the second round opens on the uplifted fee and the longer notice period, with the first reviewed run left intact.

Every proposal in this example carries confidence `1.00`, because the checked-in provider is an exact matcher and an exact match is certain. Graded confidence comes from a configured model runtime (`--runtime` / `--datum-role`), not from the offline fixture.
...
The run resource, prepared artifact identity, proposals, excerpts, and `chars:` locators are byte-stable for byte-identical inputs. Survey review-item names are not: Survey derives them from an identity hash that includes the per-invocation extraction run id Traverse mints, so the same inputs extracted into a fresh run root produce new `extraction-envelope.<hash>` item names. Reusing a run root keeps them stable, and recheck replay of the same observation pair keeps transition and item identities stable. Removing the per-invocation input from a content identity is upstream work; it is tracked in [issue #52](https://github.com/kontourai/fieldwork/issues/52).
...
`fieldwork run --task <file> (--source <file> | --snapshot <ref>)… [--snapshot-root <dir>] [--root <dir>] [--json]` creates one stable run or an ordered batch result. `fieldwork acquire --url <url> --snapshot-root <dir> [--json]` returns portable exact snapshot references. `fieldwork open <run> [--port <port>] [--theme dark|light]` binds only `127.0.0.1` and prints a launch URL whose fragment carries a random per-launch capability. Browser API requests send that capability in a header. The server allowlists loopback Host values; requires same-origin `Origin` plus `application/json` for review mutations; bounds bodies and timeouts; and returns stable public error codes without local paths. Do not share the launch URL.
...
### Read-only reviewed web sources
