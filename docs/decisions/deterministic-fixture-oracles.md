...
Both tiers still use one prepared source per run and the offline fixture adapter. Runtime conformance separately proves durable authorization, bounded concurrent completion, deterministic ordering, classified chunk failure, call ceilings, and cancellation mechanics. Multiple-source and provider-native batching, format-native document inspection, provider-quality measurement, and drift routing remain follow-up scope in [issue #9](https://github.com/kontourai/fieldwork/issues/9). Synthetic fixtures do not establish live-provider quality or justify a new extraction pass.
